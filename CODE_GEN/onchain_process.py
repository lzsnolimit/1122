...
def load_raw_chain_data(symbol: str) -> pd.DataFrame:
    """
    Reads JSON from CODE_GEN/chain/{symbol}.txt (path resolved relative to this
    module) and flattens it into a DataFrame.
    ...
    file_path = os.path.join(base_dir, "chain", f"{symbol}.txt")
