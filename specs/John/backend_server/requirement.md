...
- 成功返回：`200 OK`，JSON 数组（长度最多 10），每个元素为一个投资建议对象，至少包含以下字段：
  ...
  - `price`：数字，最新价格（单位为 `CODE_GEN/resources/{symbol}.txt` 中对应报价币种，例如 `USD`）。
- 空数据返回：`200 OK`，返回空数组 `[]`。
- 错误返回：
...
7. 每条建议对象必须包含 `price` 字段，类型为数字，表示该建议生成时的最新价格；当历史数据缺失或无法获取时可省略但需记录日志。
8. 每条建议对象的 `reason` 必须为中文（简体或繁体均可），且不得为空字符串。
9. 每条建议对象必须包含 `advice_strength` 字段，且取值仅可为 `high`、`medium`、`low` 之一。
10. 当 `predicted_at` 相同或缺失时，系统应以插入时间或主键倒序作为次级排序，保证结果稳定。
11. 当浏览器前端发起跨域 GET 请求时，系统应返回允许的 CORS 头以保证可访问性。
