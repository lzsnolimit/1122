...
2: 调用LLM分析最需要attention的symbol(get_tracking_cryptocurrenc函数返回的)，和详细的需要attention的原因。返回一个json覆盖存进social_media_analysis.txt。同时调用save_tracking_symbols_to_resources抓取这些symbol
